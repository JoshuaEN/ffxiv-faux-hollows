/**
 * This calculator uses the community data to provide candidates.
 * It simply filters out candidate patterns based on the input provided by the user (and calculated "smart fills"),
 * and then gives each tile +1 to Sword/Present for each remaining pattern with a Sword/Present.
 */
import {
...
export function calculateStatesCandidates(
  ...
  skipPattern: for (const pattern of patterns) {
    const boundingBox: ProcessedPattern["boundingBox"] = {
    ...
    for (const shape of shapes) {
      for (const index of boundingBox[shape.state].indexes()) {
        if (
      ...
      if (
        ...
        !boundingBox[shape.state].contains(shape.boundingBox)
      ) {
        continue skipPattern;
      }
    }

    filteredPatterns.push({ pattern, boundingBox });
  }
    ...
    issues.push(
      new BoardIssue(
        ...
        shapes.reduce(
          (prev, shape) => [
            ...prev,
            ...(shape.boundingBox !== null ? shape.boundingBox.indexes() : []),
          ],
          [] as number[]
        )
      )
