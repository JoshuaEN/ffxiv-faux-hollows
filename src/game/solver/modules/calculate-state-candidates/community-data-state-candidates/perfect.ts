    ...
    for (const pattern of filteredPatterns) {
      if (pattern.boundingBox.Sword.indexes().includes(index)) {
        ...
        if (found[TileState.Fox] !== true) {
          ...
          // if (
          //   pattern.pattern.ConfirmedFoxes.some(
          //     (foxIndex) =>
          //       foxIndex !== index && tiles[foxIndex] === TileState.Unknown
          //   )
          // ) {
          //   // Account for the odds of the fox being on another tile and the fox not being on this tile
          //   for (const foxIndex of pattern.pattern.ConfirmedFoxes) {
          //     if (tiles[foxIndex] === TileState.Unknown) {
